...
var rootDir = "/Users/Rishita Garg/Desktop/bee project 2/Mongo-Booking-master";
...
app.post("/booking", function (req, res) {
  ...
  if (departureDate < arrivalDate) {
    return res
      .status(400)
      .send("Depature Date cannot be before than arrival date!!");
  }
  ...
  User.find().then((users) => {
    ...
    if (
      reservedDates.includes(arrivalDate) &&
      reservedDates.includes(departureDate)
    ) {
      return res.status(400).send("Wrong Dates Selected !");
    }
  });
...
app.post("/modify", function (req, res) {
  ...
  User.findByIdAndUpdate(
    ...
    .then((user) => {
      ...
      res.send(
        "Reservation Modified for " + user.name + " for Booking ID :" + user._id
      );
    })
    .catch((err) => {
...
app.post("/delete", function (req, res) {
  ...
  User.findByIdAndRemove(id)
    .then((user) => {
      ...
      res.send(
        "Reservation Canceled for " + user.name + " for Booking ID :" + user._id
      );
    })
    .catch((err) => {
      res.status(400).send("Error in db");
